var AutoResizer = function (textArea, options) {
  ...
  this.options = $.extend({}, $.fn.autoGrow.defaults, options)
...
//    this.$textArea.change(onChange).keyup(onChange).keydown(onChange).focus(onChange);
    this.$textArea.change(onChange).keyup(onChange).focus(onChange);
  }
  ...
  checkResize: function() {
    ...
    if (!this.prevHeight || nextHeight != this.prevHeight) {
		if(nextHeight < this.options.maxHeight){
			...
			this.$textArea.css('height', this.options.maxHeight);
		}
	}
  }
};
...
$.fn.autoGrow.Constructor = AutoResizer;